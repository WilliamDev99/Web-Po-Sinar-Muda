...
export default function AdminEditArmadaPage() {
  ...
  useEffect(() => {
    async function fetchArmada() {
      ...
      if (error) {
        alert("Gagal memuat data armada: " + error.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Perubahan armada berhasil disimpan!");
      router.push('/admin/armada');
    } catch (err) {
      alert(err.message);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-2xl mx-auto w-full">
      <div className="flex items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Edit Armada</h2>
          <p className="text-sm text-gray-500">Ubah informasi armada dan kelengkapan fasilitasnya</p>
        </div>
      </div>
