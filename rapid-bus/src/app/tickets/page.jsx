  ...
  return (
    <>
      ...
      <main className="pt-32 pb-32 md:pb-16 px-4 md:px-10 max-w-lg md:max-w-4xl mx-auto relative z-10 flex flex-col gap-6">
        ...
        {/* Header Title */}
        <h2 className="text-3xl font-bold text-white text-center md:text-left mb-2 md:mb-6 drop-shadow-sm">Tiketku</h2>
        ...
        <div className="flex flex-col gap-5 mt-2">
          {tickets.length > 0 ? (
            tickets.map((ticket) => (
              <div key={ticket.id} className="bg-white/95 backdrop-blur-md rounded-[28px] shadow-[0_15px_40px_rgba(0,0,0,0.1)] border border-white/40 overflow-hidden flex flex-col group hover:shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-300">
                {/* Header (Status & Code) */}
                <div className="bg-gradient-to-r from-[#1f75b8] to-[#60a5fa] p-4 md:p-5 flex justify-between items-center text-white">
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[20px]">{activeTab === 'aktif' ? 'schedule' : 'check_circle'}</span>
                    <span className="font-bold text-sm">{ticket.status}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-white/80 font-medium uppercase tracking-wider">Kode Booking</span>
                    <span className="font-mono font-bold text-base md:text-lg">{ticket.id}</span>
                  </div>
                </div>
                ...
                <div className="p-5 md:p-6 flex flex-col gap-5">
                  <div className="flex justify-between items-center border-b border-gray-100 pb-5">
                    <div className="flex flex-col">
                      <span className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Rute Perjalanan</span>
                      <span className="font-bold text-gray-800 text-lg md:text-xl">{ticket.route}</span>
                      <span className="text-sm font-semibold text-[#1f75b8] mt-0.5">{ticket.busType}</span>
                    </div>
                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-[#1f75b8]/10 flex items-center justify-center text-[#1f75b8] shrink-0">
                      <span className="material-symbols-outlined text-[24px] md:text-[32px]">directions_bus</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] text-gray-400 font-medium uppercase flex items-center gap-1">
                        <span className="material-symbols-outlined text-[14px]">calendar_today</span> Tanggal
                      </span>
                      <span className="font-bold text-gray-800 text-sm md:text-base">{ticket.date}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] text-gray-400 font-medium uppercase flex items-center gap-1">
                        <span className="material-symbols-outlined text-[14px]">schedule</span> Jam
                      </span>
                      <span className="font-bold text-gray-800 text-sm md:text-base">{ticket.time}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] text-gray-400 font-medium uppercase flex items-center gap-1">
                        <span className="material-symbols-outlined text-[14px]">group</span> Penumpang
                      </span>
                      <span className="font-bold text-gray-800 text-sm md:text-base">{ticket.passengers.length} Orang</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] text-gray-400 font-medium uppercase flex items-center gap-1">
                        <span className="material-symbols-outlined text-[14px]">event_seat</span> Kursi
                      </span>
                      <span className="font-bold text-gray-800 text-sm md:text-base">{ticket.seats.join(', ')}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-[28px] shadow-[0_15px_40px_rgba(0,0,0,0.1)] border border-white/40 p-10 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">Belum ada tiket {activeTab}</h3>
              <p className="text-gray-500 mb-6 max-w-xs">Anda belum memiliki tiket yang {activeTab}. Yuk, cari tiket untuk perjalanan Anda berikutnya!</p>
              <Link href="/" className="bg-[#1f75b8] hover:bg-[#19619c] text-white font-bold py-3 px-8 rounded-xl transition-colors shadow-md">
                Cari Tiket
              </Link>
            </div>
          )}
      ...
      <nav className="md:hidden fixed bottom-0 w-full z-50 rounded-t-3xl bg-white/90 backdrop-blur-xl border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.08)] flex justify-around items-center h-20 px-6 pb-2">
        ...
        {/* My Bookings (Active) */}
        <Link className="flex flex-col items-center justify-center gap-1 bg-[#a3d1ff]/10 text-[#1f75b8] rounded-2xl px-5 py-2 hover:bg-[#a3d1ff]/20 active:scale-95 transition-all duration-300" href="/tickets">
          <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>confirmation_number</span>
          <span className="text-[11px] font-bold">Tiketku</span>
        </Link>
        {/* Profile */}
